...
# possible changes: drop taxes and natural resource share to increase observations; some variables with very high vif
...
# df_simple_cross = df.dropna(subset=["net_fdistock", "gti_scaled"])
#
...
#     X = sm.add_constant(group["gti_scaled"])
#     model = sm.OLS(y,X).fit()
#
#     coef = model.params["gti_scaled"]
#     ci_low, ci_high = model.conf_int().loc["gti_scaled"]
#
...
df_multiple_cross = df_multiple_cross.dropna(subset=["net_fdistock", "gti_scaled"] + controls + region_dummy_names)
...
for year, group in df_multiple_cross.groupby("year"):
    ...
    y = group["net_fdistock"]
    X = sm.add_constant(group[["gti_scaled"]+ controls + region_dummy_names])
    model = sm.OLS(y,X).fit()
