...
program define resid_ar
    syntax varlist(min=1 max=1), OTHERs(string) COND(string) REFLAG(integer) ///
        RESULTNAME(string)

    local dv : word 1 of `varlist'

    cap qui xtreg `dv' L(1/`reflag').`dv' L(1/`reflag').(`others') i.t ///
        if `cond', fe
    if _rc {
        di as error "  LSDV failed for `dv' (rc " _rc ")"
        exit
    }
    ...
    di as txt _n "  Equation: `dv'   (controls: `others', lags 1-`reflag')"
    di as txt "    lag        b         se          t"
    forvalues j = 1/8 {
        cap qui xtreg _resid_ar L`j'._resid_ar if `cond', fe
        if _rc continue
        local b  = _b[L`j'._resid_ar]
        local se = _se[L`j'._resid_ar]
        local tt = `b' / `se'
...
foreach cell of global CHECKCELLS {
    ...
    * Terror variables of this specification, in order.
    ...
    if `nterr' == 2 {
        ...
        resid_ar `v1', others(`v2' y_fdi) cond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc)
        resid_ar `v2', others(`v1' y_fdi) cond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc)
        resid_ar y_fdi, others(`v1' `v2') cond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc)
    }
    else {
        local v1 : word 1 of `tvlist'
        resid_ar `v1', others(y_fdi) cond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc)
        resid_ar y_fdi, others(`v1') cond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc)
    }
}
postclose rc
