...
program define resid_ar
    syntax varlist(min=1 max=1), OTHERVARS(string) SUBCOND(string) ///
        REFLAG(integer) RESULTNAME(string) CELLID(string)
    ...
    * variable separately instead.
    ...
    forvalues j = 1/8 {
        ...
        local dvname = "`dv'"
        post `resultname' (`"`cellid'"') (`"`dvname'"') (`j') (`b') (`se') (`tt')
    }
    cap drop _resid_ar
end
...
postfile rc str10 cellid str20 eqvar int lag double(b se t) ///
    ...
    * Terror variables of this specification, in order.
    ...
    if `nterr' == 2 {
        ...
        resid_ar `v1', othervars(`v2' y_fdi) subcond(insample & (${COND_`sm'})) ///
            reflag(`rl') resultname(rc) cellid(`cellid')
