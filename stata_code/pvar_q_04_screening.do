...
global STAMP "run1"      // change to keep several screening runs side by side
...
*--- 1a. Base sample ---------------------------------------------------------*
global BASESAMPLE   "core"     // full | nospe | core
global MINOBS       28
...
global FDIVARS `" "net|fdi_net_pct_qgdp|1" "musd|fdi_inflow_musd|1" "'
...
* variables from 1e, any variable in the data, or an explicit list.
global SAMPLES `" "all|1" "rich|g_inc==3" "poor|g_inc==1" "midinc|g_inc==2" "terrhigh|g_terr==3" "terrlow|g_terr==1" "'
...
if $RUN_STAGE1 == 1 {
    ...
    foreach v of local ALLVARS {
        ...
        *--- CIPS, levels and first differences ---
        ...
        cap qui pescadf `v' if insample & _sdv > 0 & !missing(_sdv), lags(4) trend
        if !_rc {
        ...
        cap drop _dv
        gen double _dv = D.`v'
        cap qui pescadf _dv if insample & _sdv > 0 & !missing(_sdv), lags(4) trend
        if !_rc {
            foreach nm in zt_bar cips tbar Zt_bar {
                if missing(`cdif') {
                    cap local cdif = r(`nm')
